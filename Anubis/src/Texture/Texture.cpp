...
bool Texture::load_texture()
{
	...
	unsigned char* tex_data = stbi_load(filepath.c_str(), &width, &height, &bit_depth, 0);
	if (!tex_data)
	{
...
bool Texture::load_textureA()
{
	...
	unsigned char* tex_data = stbi_load(filepath.c_str(), &width, &height, &bit_depth, force_channels);
	if (!tex_data)
	{
		std::cout << "Could not find: " << filepath << "\n";
		return false;
...
bool Texture::load_texture_grayscale()
{
	stbi_set_flip_vertically_on_load(true);
	...
	glTexImage2D(m_textureTarget, 0, GL_RED, width, height, 0, GL_RED, GL_UNSIGNED_BYTE, tex_data);
